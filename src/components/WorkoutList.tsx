      ...
      {workouts.length === 0 ? (
        ...
        <div className="space-y-4">
          {workouts.map((workout, workoutIndex) => (
            <div key={workoutIndex} className="border rounded p-4">
              <h3 className="font-semibold mb-2">Date: {workout.date}</h3>
              <ul className="space-y-2">
                {workout.exercises.map((exercise, exerciseIndex) => (
                  <li key={exerciseIndex} className="p-2 bg-gray-50 rounded">
                    <h4 className="font-medium">{exercise.name} ({exercise.type})</h4>
                    {exercise.type === 'reps' ? (
                      <>
                        <p>Target: {exercise.targetSets} sets / {exercise.targetReps} reps</p>
                        <p>Completed: {exercise.completedSets} sets / {exercise.completedReps} reps</p>
                      </>
                    ) : (
                      <>
                        <p>Target Weight: {exercise.targetWeight} kg</p>
                        <p>Completed Weight: {exercise.completedWeight} kg</p>
                      </>
                    )}
                  </li>
                ))}
